...
'''
        if numTurns == 0:
            if math.fabs(meandiff) < 25:
                targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 40, maxShift)
                pitchAdj = round((targetVal / basepitch), 2)
        else:
            if math.fabs(meandiff) < 5:
                pitchAdj = defaultShift
            elif numTurns < 3:
                if math.fabs(meandiff) > 20:
                    targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 25, maxShift)
                    pitchAdj = round( (targetVal / basepitch),2)
                else:
                    pitchAdj = defaultShift
            elif numTurns == 3:
                if math.fabs(meandiff) > 10:
                    targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 15, maxShift)
                    pitchAdj = round( (targetVal / basepitch),2)
                else:
                    pitchAdj = defaultShift
            elif numTurns > 3 and numTurns < 6:
                if math.fabs(meandiff) > 5:
                    targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 5, maxShift)
                    pitchAdj = round((targetVal / basepitch), 2)
                else:
                    targetVal = userMean
                    shift = 100
                ...
                targetVal = userMean
                shift = 100
                pitchAdj = round((userMean / basepitch),2)
'''
