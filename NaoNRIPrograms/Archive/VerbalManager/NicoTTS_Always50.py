...
'''
        if numTurns == 0:
            if math.fabs(meandiff) < 25:
                targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 25)
                pitchAdj = round((targetVal / basepitch), 2)
            else:
                targetVal = nicoMean
                shift = 0
            ...
            if numTurns < 3:
               targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 15)
               pitchAdj = round( (targetVal / basepitch),2)
            elif numTurns > 2 and numTurns < 5:
               targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 10)
               pitchAdj = round( (targetVal / basepitch),2)
            elif numTurns > 5 and numTurns < 8:
               targetVal, shift = getTargetVal(meandiff, userMean, nicoMean, 5)
               pitchAdj = round((targetVal / basepitch), 2)
            else:
                targetVal = userMean
                shift = 0
